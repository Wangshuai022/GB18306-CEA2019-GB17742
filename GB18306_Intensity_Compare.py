...
def main():
    """计算、导出并绘制两种烈度估计方法的距离衰减对比。

    本入口读取模块顶部的 ``REGION``、``MS``、距离范围、距离采样数及经验调整量
    ``adjust``，分别沿椭圆长轴和短轴计算：GB18306 直接烈度、由 GB18306
    PGA/PGV 经 GB/T 17742 换算的仪器烈度，以及二者差值。

    Returns
    -------
    None
        结果写入 ``Test_output``：一个逐距离 CSV 和一幅 2×2 PNG；终端同时打印
        1、10、50、100、200、400 km 的摘要。本函数不修改模型系数。

    Notes
    -----
    这是面向脚本运行的对比工具。若要把结果嵌入其他程序，应优先直接调用
    ``GB18306_class.py`` 和 ``GB17742_class.py`` 中的模型类。
    """
    # ---------- 计算 ----------
    data = {}
    for axis in AXES:
