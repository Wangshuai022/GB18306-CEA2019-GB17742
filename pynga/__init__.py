...
def NGA08(
    ...
    if period > 10.0 or 0 < period < 0.01:
        print(
            "Positive period value should be within [0.01,10] "
    ...
    if itmp == 0:
        ...
        values_low = np.array(
            mapfunc(ngaM, Mw, Rjb, Vs30, period_low, rake, **kwds)
        )
        values_high = np.array(
            mapfunc(ngaM, Mw, Rjb, Vs30, period_high, rake, **kwds)
        )
...
def BA08Test(T):
    ...
    lines = []
    for Mw in Mws:
        median, std, tau, sigma = NGA08(
            nga, Mw, Rjb, Vs30, T, Mech=1, NGAs=NGAs
        )
...
def NGA14(
    ...
    if NGAs == None:
        NGAs = {
            "CB": {"NewCoefs": None, "terms": (1, 1, 1, 1, 1, 1, 1, 1, 1)},
...
def BSSA14_validation(infile, outfile, iset):
    ...
    ftNGAs = np.array(
        [inputs["Y(g)"], inputs["sigma"], inputs["tau"], inputs["phi"]]
    )
    # plot
    fig = plt.figure(1)
